...
def lammps_data(input_file, data_file):
  ...
  f.write("\n %f %f %s"%(0.00000, v2[0][0], 'xlo xhi'))
  f.write("\n %f %f %s"%(0.00000, v2[1][1], 'ylo yhi'))
  ...
  for i in range(natom):
    ...
    if pos_A_z[i] > z_mean: 
      indx = basis2.index(at_type.tolist()[i].decode()) + 1 + len(np.unique(basis_1)) # bottom layer atom type decoded
