...
lammps_relax=true
forcefield='../../../aux_files/ff'
style=''
lammps_path='~/Codes/lammps-3Mar20/src/lmp_stampede'
#-------------------------------------------------
#-------------------------------------------------
...
if [ ! -f $hextable/hex.table ]; then
  ...
  printf "$latcon" | python $aux_files/hex_table_least.py 
...
EOF
        ...
        python $twister_path/twister.py twist.inp
        ...
        if [ "$lammps_relax" = true ]; then
          ...
          if [[ $style = use_python ]]; then
            ...
            cp $aux_files/extract_pos.py ./
            ibrun -n 1 $lammps_path -in lammps.in
            python extract_pos.py
          fi
        fi
        cd ../
